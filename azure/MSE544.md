...
* we would like to better understand Python interpreters and Mac compatibility
    * Python 3.8+ 64-bit; env vars PATH, PYTHONPATH, etc
...
- wildflowers (data: PlanetLabs images of meadows on the flanks of Mt. Rainier)
...
- Return to the Azure Function App and navigate to **Functions** (left sidebar): No Functions listed
    - When we try to create a Function to go here: The portal advises us to go install and use VSCode
...
- Remember to stop and re-start VSCode to cause changes to take effect
...
#### TA team: Skip the following to test whether it is necessary
    
    
- kilroy need to clarify whether a Python install is needed, which version (3.9.12?) etc.
    - The following is from the old version of this walkthrough:
        ...
        - Run the "Python: Select interpreter" command (kilroy make sure this Python matches the Azure interpreter choice)   
...
> Two kilroy items: Python 3.8? Also `AZFN1` is fine but could be replaced with a better name.
...
* From the VSCode **`Run`** menu: Start the debugger
    * This will launch the Azure Function App locally on your laptop
