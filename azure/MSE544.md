...
- [Slides kilroy to pdf to Luna](https://docs.google.com/presentation/d/1R2HHfHYgmarje2q_0x42hMecUc7R6SFjI-ih1L05foQ/edit#slide=id.p)
...
- Install the [Azure Functions Extension in VSCode](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurefunctions)
    - kilroy sort out [this link](https://learn.microsoft.com/en-us/azure/azure-functions/functions-versions?tabs=v4&pivots=programming-language-python) vis-a-vis runtimes
- You should not need to do this (avoid random suggestions)
    - **`pip install azure-functions`**
    - [**`pip install azure-core`**](https://pypi.org/project/azure-core/)
    - ...because there is **`requirements.txt`**, the way to go
- Mac: Install [.Net Core](https://dotnet.microsoft.com/download) kilroy test is this necessary and does it work
...
<img src="https://github.com/robfatland/serverless/blob/main/azure/images/kilroy.png" alt="drawing" width="600"/>
...
- Paste the URL to a new browser tab to arrive at a simple placeholder for your Azure Function App 
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    logging.info('Python HTTP trigger function processed a request, hurray')
    n = req.params.get('n')
    s = req.params.get('s')
    if n:
...
It remains to use VSCode to deploy the Azure Function to the Azure cloud.
This is a two-step process: Create a Azure Function App--which is a landing space--and
then put the Azure Function in that space. 
...
- From here expand the **`Function App`** category (lightning bolt icon)
    - There may be one or more Function Apps listed here
- Right click on **`Function App`** and select **`Create Function App in Azure`**
    ...
    - When you view the output: Notice a URL is assigned to your new Function App
        - This is *not* the URL of your Azure Function.
    
    
Step two: Now that we have the Function App landing space we can upload the Azure Function.
    
    
- Ensure you are in the VSCode Azure view
- Place your cursor inside the **`WORKSPACE`** panel and select **`Deploy...`** (up-arrow + cloud icon)
    - Select **`Deploy to Function App`**
    - Select the folder for your Azure Function
    - Select the Azure Function App resource from the previous step above
...
Next we will run the Azure Function using about four lines of Python code. 


kilroy do students need to `python -m pip install requests` in some manner?
