...
There is a lot of content here. Your walkthrough *tasks* are bullet lists starting 
with [Section 1 on Azure access](#1-establish-access-to-an-azure-subscription). The
explanatory material...


> ...tends to sit in offset paragraphs like this. 
 
 
There are ten sections to go through (see the [table of contents](#table-of-contents)) 
followed by some extra material in Appendices at the end.
...
Per TA discussion on 4/20/23 we would like to understand the '...also install...' needs to build a working 
ecosystem. MSE544 students: Please skip this section.
...
Remaining open questions to sort out; this bullet list will vanish once resolved. 


* Add an optional test of the localhost version via Python: Caveat debugger must be running
* Go through this walkthrough and make all informational text offset > vs task is bullet; then write this at the top
* Azure Function components: List and particularly what is the storage account for?
...
> If you start accessing PlanetLabs RGB+NearIR images of meadows on Mount Rainier: You can write an
> automated Azure Function to identify wildflower blooms. You could use Azure Functions to standardize
> file formats for people uploading multimedia content to the cloud. What other uses? Well... database
> modification quality control; Internet of Things converting sensor streams to actuator devices. 
> Route traffic to chat bots by scaling up / down based on demand. This is the tip of the iceberg; 
> feel free to search on more applications of serverless computing.
