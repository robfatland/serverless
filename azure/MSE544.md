...
with [Section 1 on Azure access](#1-establish-access-to-an-azure-subscription). 
...
> Suppose you start harvesting data from a company called Planet. They operate about 150 satellites,
> each generating images of the earth's surface at high resolution. In fact
> each day they generate 350 million square kilometers of image data; where each pixel has four values: 
> Red, Green, Blue, and Near-Infrared. Suppose you are interested in flower blooms on alpine
> meadows. Your workflow consists of (1) identifying candidate images (2) analyzing the color signature
> at each pixel for wildflower bloom signatures and (3) writing results to a database for additional 
> analysis at a later time. 
> This workflow could be built as a collection of Azure Functions, working away tirelessly to 
> generate the data that you are particularly interested in.
> 
> Azure Functions (serverless functions in general) have many other uses. They can standardize
> file formats for people uploading multimedia content to the cloud. 
> Database quality control; Internet of Things converting sensor streams to actuator devices. 
