...
kilroy update this (aye means is-current)



- aye: [Microsoft Azure Functions overview](https://learn.microsoft.com/en-us/azure/azure-functions/)
...
A quick look at what that top row is all about...


- **Create a resource** is the starting point for manually getting cloud resources, also called *services*. 
- **Resource group** is a virtual container (think 'project box') for related resources we use to build something with
- **Subscription** is a User account; it can contain many Resource groups
- **Marketplace** is a catalog of cloud resources
- **Storage account** is like a Resource group specifically dedicated to different types of data storage
...
Type 'function' in the search window at the top to zero in on Azure Function App (the full name of the Azure Function
service). Select Function App from the resulting list of Services. 
...
Click on the **Create** button. We proceed to a *create* wizard that eventually builds an Azure Function. 
...
kilroy left off here on portal work section 2
