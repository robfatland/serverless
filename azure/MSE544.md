...
The four lines of code below send an HTTP message to your Azure Function, get 
a reply, and print the text of that reply. 
...
You can then copy-paste the code. You can stop Python by typing `exit()`.

    
The alternative is to save this code as a file 
with a name like `testFunction.py`; and then run the file from the bash command line: 
`python testFunction.py`. 
 

```
import requests
urlbase='https://function-app-from-robs-laptop.azurewebsites.net/api/HttpTrigger1'
