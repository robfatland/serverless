...
- While the debugger is still running (so the local Azure Function is available)...
    - Return to the code editor and click to the left of a line of Python code as in the screencap below
        - This places a stop in the code
    - Place the Function URL in your browser again; append a new value of the parameter `n`: **`?n=1234`**
    - Hit Enter and notice that VSCode comes up indicating the code has stopped
